...
@app.route('/api/admin/users/generate-password', methods=['POST'])
@csrf.exempt
@login_required
@admin_required
def api_admin_generate_password():
    """Сгенерировать пароль и сохранить в отдельную таблицу (user_id пока NULL, привязка при создании пользователя)."""
    try:
        password = auth_service.generate_random_password()
        record_id = auth_service.save_generated_password(password, user_id=None)
        if record_id is None:
            return jsonify({'success': False, 'error': 'Ошибка сохранения пароля'}), 500
        return jsonify({'success': True, 'password': password, 'record_id': record_id})
    except Exception as e:
        logger.error(f"Ошибка генерации пароля: {e}", exc_info=True)
...
def api_admin_create_user():
    """Создать пользователя. В теле можно передать generated_password_record_id для привязки сгенерированного пароля."""
    try:
        ...
        record_id = data.get('generated_password_record_id')
        if not username or not email or not password:
            return jsonify({'success': False, 'error': 'Укажите имя, email и пароль'}), 400
        if role not in ('admin', 'user'):
            role = 'user'
        user_id = auth_service.create_user(username=username, email=email, password=password, role=role, full_name=full_name)
        if user_id is None:
            return jsonify({'success': False, 'error': 'Не удалось создать пользователя (возможно, email уже занят)'}), 400
        if record_id is not None:
            auth_service.link_generated_password_to_user(int(record_id), user_id)
...
def api_admin_get_user(user_id):
    """Получить данные пользователя для редактирования (включая сохранённый сгенерированный пароль, только для админа)."""
    try:
        user = auth_service.get_user_by_id(user_id)
        if not user:
            return jsonify({'success': False, 'error': 'Пользователь не найден'}), 404
        generated_password = auth_service.get_generated_password(user_id)
...
def api_admin_update_user(user_id):
    """Обновить пользователя. Пароль опционален; если передан — обновляется хеш и запись в user_generated_passwords."""
    try:
        data = request.get_json() or {}
        user = auth_service.get_user_by_id(user_id)
        if not user:
        ...
        role = (data.get('role') or user['role'] or 'user').strip()
        full_name = (data.get('full_name') or user.get('full_name') or '').strip() or None
        ...
        if password:
            ...
            db_manager.execute_query(
                "UPDATE users SET username=%s, email=%s, role=%s, full_name=%s, is_active=%s, password_hash=%s, updated_at=%s WHERE id=%s",
                (username, email, role, full_name, is_active, password_hash, datetime.utcnow(), user_id)
            )
            auth_service.update_generated_password(user_id, password)
        else:
            db_manager.execute_query(
                "UPDATE users SET username=%s, email=%s, role=%s, full_name=%s, is_active=%s, updated_at=%s WHERE id=%s",
                (username, email, role, full_name, is_active, datetime.utcnow(), user_id)
            )
        return jsonify({'success': True})
    except Exception as e:
        logger.error(f"Ошибка обновления пользователя {user_id}: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_admin_delete_user(user_id):
    """Удалить пользователя (админ). Нельзя удалить себя."""
    if user_id == session.get('user_id'):
        return jsonify({'success': False, 'error': 'Нельзя удалить себя'}), 400
    try:
        db_manager.execute_query("DELETE FROM user_vulnerability_assignments WHERE user_id = %s", (user_id,))
