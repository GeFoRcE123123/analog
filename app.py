...
@app.route('/import-unified')
@login_required
def import_unified_page():
    """Унифицированная страница импорта XML/JSON"""
    return render_template('import_unified.html')
...
# ========== ИМПОРТ/ЭКСПОРТ XML/JSON ==========

@app.route('/api/import/unified', methods=['POST'])
@login_required
def api_import_unified():
    """API для импорта XML/JSON файлов"""
    try:
        ...
        # Определяем формат
        format_type = request.form.get('format', 'xml')
        skip_duplicates = request.form.get('skip_duplicates', 'true') == 'true'
        auto_generate_cve = request.form.get('auto_generate_cve', 'false') == 'true'
        
        # Проверяем расширение
        filename = file.filename.lower()
        if format_type == 'xml' and not filename.endswith('.xml'):
            return jsonify({'success': False, 'error': 'Ожидается XML файл'}), 400
        elif format_type == 'json' and not filename.endswith('.json'):
            return jsonify({'success': False, 'error': 'Ожидается JSON файл'}), 400
        ...
        if format_type == 'xml':
            ...
            try:
                ...
                for vuln_elem in root.findall('.//vulnerability'):
                    vuln = parse_xml_vulnerability(vuln_elem, 'owasp')
                    if vuln:
                        vulnerabilities.append(vuln)
                
                if not vulnerabilities:
                    for vuln_elem in root.findall('.//item'):
                        vuln = parse_xml_vulnerability(vuln_elem, 'custom')
                        if vuln:
                            vulnerabilities.append(vuln)
                
                if not vulnerabilities:
                    for vuln_elem in root.findall('.//entry'):
                        vuln = parse_xml_vulnerability(vuln_elem, 'nvd')
                        if vuln:
                            vulnerabilities.append(vuln)
                            
            except ET.ParseError as e:
                return jsonify({'success': False, 'error': f'Ошибка парсинга XML: {str(e)}'}), 400
            ...
            try:
                ...
                if isinstance(data, list):
                    for item in data:
                        vuln = parse_json_vulnerability(item)
                        if vuln:
                            vulnerabilities.append(vuln)
                
                # Формат 2: Объект с ключом vulnerabilities
                    ...
                    if 'vulnerabilities' in data and isinstance(data['vulnerabilities'], list):
                        ...
                        # Одна уязвимость
                        vuln = parse_json_vulnerability(data)
                        if vuln:
                            vulnerabilities.append(vuln)
        ...
        if not vulnerabilities:
            return jsonify({'success': False, 'error': 'Не найдено уязвимостей в файле'}), 400
        
        # Сохраняем в БД
        imported, skipped = save_vulnerabilities_to_db(vulnerabilities, skip_duplicates, auto_generate_cve)
        
        return jsonify({
            'success': True,
            'message': f'Импортировано {imported} уязвимостей, пропущено {skipped} дубликатов',
            'imported': imported,
            'skipped': skipped,
...
def api_export_json():
    ...
    try:
        ...
        query = """
            SELECT id, source, cve, title, cvss, etc, joining_date, status
            FROM turn 
        ...
        query += " ORDER BY cvss DESC, joining_date DESC LIMIT %s"
        params.append(limit)
        ...
        for row in results:
            try:
                vuln = {
                    ...
                    'cve_id': row[2] or '',
                    'title': row[3] or '',
                    'cvss_score': float(row[4]) if row[4] else 0.0,
                    'status': 'active' if row[7] else 'completed',
                    'joining_date': row[6].isoformat() if row[6] else '',
                    'severity': 'critical' if row[4] and row[4] >= 9.0 else 
                               'high' if row[4] and row[4] >= 7.0 else 
                               'medium' if row[4] and row[4] >= 4.0 else 'low'
                }
                
                # Парсим etc если есть
                if row[5]:
                    try:
                        etc_data = json.loads(row[5])
                        vuln['description'] = etc_data.get('description', '')
                        vuln['remediation'] = etc_data.get('remediation', '')
                        vuln['tags'] = etc_data.get('tags', [])
                    except:
                        vuln['description'] = row[5]
                
                vulnerabilities.append(vuln)
            except Exception as e:
                logger.error(f"Ошибка обработки записи {row[0]}: {e}")
                continue
        ...
        response = Response(
            ...
            headers={
                'Content-Disposition': f'attachment; filename=vulnerabilities_export_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
            }
        )
...
def api_export_xml():
    ...
    try:
        ...
        # Получаем данные из БД (используем ту же логику что и для JSON)
        query = """
            SELECT id, source, cve, title, cvss, etc, joining_date, status
            FROM turn 
            WHERE cve IS NOT NULL AND cve != ''
        ...
        root = ET.Element('vulnerability-report')
        
        # Метаданные
        metadata = ET.SubElement(root, 'metadata')
        ET.SubElement(metadata, 'export-date').text = datetime.now().isoformat()
        ET.SubElement(metadata, 'total-vulnerabilities').text = str(len(results))
        ET.SubElement(metadata, 'format').text = 'OWASP-Compatible'
        ET.SubElement(metadata, 'generated-by').text = 'Vulnerability Management System'
        ...
        for row in results:
            try:
                ...
                ET.SubElement(vuln_elem, 'id').text = str(row[0])
                ET.SubElement(vuln_elem, 'source').text = row[1] or 'unknown'
                ET.SubElement(vuln_elem, 'cve-id').text = row[2] or ''
                ET.SubElement(vuln_elem, 'title').text = row[3] or ''
                ET.SubElement(vuln_elem, 'cvss-score').text = str(float(row[4]) if row[4] else 0.0)
                ET.SubElement(vuln_elem, 'status').text = 'active' if row[7] else 'completed'
                ET.SubElement(vuln_elem, 'joining-date').text = row[6].isoformat() if row[6] else ''
                
                # Парсим etc если есть
                if row[5]:
                    try:
                        etc_data = json.loads(row[5])
                        description = etc_data.get('description', '')
                        if description:
                            ET.SubElement(vuln_elem, 'description').text = description
                    except:
                        pass
                        
            except Exception as e:
                logger.error(f"Ошибка обработки записи для XML: {e}")
                continue
        ...
        response = Response(
            ...
            headers={
                'Content-Disposition': f'attachment; filename=vulnerabilities_export_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xml'
            }
        )
        ...
        logger.error(f"Ошибка экспорта XML: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def parse_xml_vulnerability(vuln_elem, format_type):
    """Парсит уязвимость из XML элемента"""
    try:
        vuln = {
            'source': 'xml_import',
            'title': '',
        ...
        if format_type == 'owasp':
            vuln['title'] = vuln_elem.findtext('title', '')
            vuln['cve'] = vuln_elem.findtext('cve-id', '') or vuln_elem.findtext('cve', '')
            description = vuln_elem.findtext('description', '')
            
            severity = vuln_elem.findtext('severity', 'medium')
            ...
            cvss_elem = vuln_elem.find('cvss-score') or vuln_elem.find('cvss')
            ...
            vuln['title'] = vuln_elem.findtext('name', '') or vuln_elem.get('name', '')
            vuln['cve'] = vuln_elem.findtext('cve', '') or vuln_elem.get('cve', '')
            description = vuln_elem.findtext('details', '') or vuln_elem.findtext('description', '')
            
        elif format_type == 'nvd':
            vuln['title'] = vuln_elem.findtext('summary', '')
            vuln['cve'] = vuln_elem.findtext('cve', '')
            description = vuln_elem.findtext('description', '')
            
            cvss_elem = vuln_elem.find('cvss')
            if cvss_elem is not None:
                try:
                    vuln['cvss'] = float(cvss_elem.text)
                except:
        ...
        # Автогенерация CVE если нужно
        if not vuln['cve']:
            vuln['cve'] = f"TEMP-{str(uuid.uuid4())[:8]}"
        ...
        vuln['etc'] = json.dumps(etc_data, ensure_ascii=False)
        
        # Вычисляем приоритет если не задан
        if vuln['priority'] == 0.0:
            if vuln['cvss'] >= 9.0:
                vuln['priority'] = 1.0
            elif vuln['cvss'] >= 7.0:
                vuln['priority'] = 0.8
        ...
        logger.error(f"Ошибка парсинга XML элемента: {e}")
        return None

def parse_json_vulnerability(item):
    """Парсит уязвимость из JSON объекта"""
    try:
        vuln = {
            'source': 'json_import',
            'title': item.get('title') or item.get('name') or '',
            'cve': item.get('cve') or item.get('cve_id') or item.get('id') or '',
            'cvss': float(item.get('cvss_score', item.get('cvss', 0.0))),
            'etc': '',
            'joining_date': datetime.now(),
            'status': True,
            'priority': 0.0,
        ...
        etc_data = {
            ...
            'affected_systems': item.get('affected_systems') or item.get('systems') or [],
        ...
        logger.error(f"Ошибка парсинга JSON объекта: {e}")
        return None

def save_vulnerabilities_to_db(vulnerabilities, skip_duplicates=True, auto_generate_cve=False):
    """Сохраняет уязвимости в БД"""
    imported = 0
    skipped = 0
    ...
    for vuln in vulnerabilities:
        try:
            # Проверяем дубликаты
            if skip_duplicates and vuln['cve']:
                check_query = "SELECT COUNT(*) FROM turn WHERE cve = %s"
                result = db_manager.execute_query(check_query, (vuln['cve'],))
                if result and result[0][0] > 0:
                    logger.info(f"Дубликат CVE {vuln['cve']}, пропускаем")
                    skipped += 1
                    continue
            
            # Автогенерация CVE если нужно
            if auto_generate_cve and (not vuln['cve'] or vuln['cve'].startswith('TEMP-')):
                vuln['cve'] = f"TEMP-{str(uuid.uuid4())[:8]}"
            
            # Вставляем запись
            query = """
                INSERT INTO turn (
                    source, title, cve, cvss, etc, joining_date, 
                    status, priority, price_one, name
                ) VALUES (%s, %s, %s, %s, %s, NOW(), %s, %s, %s, %s)
            """
            
            # name берем из title
            name = vuln['title'][:100] if vuln['title'] else 'Unknown'
            ...
            imported += 1
            logger.debug(f"Сохранена уязвимость: {vuln['cve']}")
            
        except Exception as e:
            logger.error(f"Ошибка сохранения уязвимости {vuln.get('cve', 'N/A')}: {e}")
            continue
    
    logger.info(f"Импорт завершен: сохранено {imported}, пропущено {skipped}")
